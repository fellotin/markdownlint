    ...
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        ...
        try {
          for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
            var row = _step2.value;
            var cells = filterByTypes(row.children, ["tableContent"]);
            var actualCount = cells.length;
            expectedCount || (expectedCount = actualCount);
            addErrorDetailIf(onError, row.endLine, expectedCount, actualCount, "Too ".concat(actualCount > expectedCount ? "many" : "few", " columns"));
          }
        } catch (err) {
          _iterator2.e(err);
        } finally {
